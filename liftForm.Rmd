...
``` {r featCorr,echo=FALSE,fig.height=3,fig.width=7}
...
for(i in 1:3) {
        ...
        allVar_err <- round((1 - sum(diag(allVar_rfFit$test$confusion))) / nrow_train_ML, 5)
        exCorr_err <- round((1 - sum(diag(exCorr_rfFit$test$confusion))) / nrow_train_ML, 5)
        allVar_PCA_err <- round((1 - sum(diag(allVar_PCA_rfFit$test$confusion))) / nrow_train_ML, 5)
...
df_error <- df_error %>%
        mutate(
                avgError = (modelError1 + modelError2 + modelError3) / 3
        )
